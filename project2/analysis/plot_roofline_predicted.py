#!/usr/bin/env python3
"""
plot_roofline_predicted.py  –  Roofline chart using hierarchical model predictions.
This mirrors project1/part1/plot_roofline.py but uses predicted AI and GFLOPS
from the calibrated hierarchical model (primary mode).
"""
...
OUT = os.path.join("..", "results", "roofline_predicted.png")
...
# Marker and color mapping (match part1 plot)
MARKERS = {
    "naive": "o",
    "smem_w": "s",
    "unroll": "^",
...
# Filled status used in original plot (keep consistent)
FILLED = {"naive": True, "smem_w": False, "unroll": True, "smem_wi": False, "regblock": True, "warp": True}

# Label offsets copied from original plot_roofline.py
LABEL_CFG = {
...
def build_data_from_predictions():
    out = hierarchical.evaluate(hierarchical.default_results_dir())
    rows = out["primary_mode_rows"]
    data = {}
    for r in rows:
        key = (r["cfg"], r["kernel"])
...
def main():
    DATA = build_data_from_predictions()
